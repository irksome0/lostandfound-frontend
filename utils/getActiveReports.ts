...
import { ActiveReports, ReportsResponse } from "@/typings"

export const getActiveReports = async () => {
    const jwt = cookies().get("jwt")
    
    const response = await fetch("http://127.0.0.1:8800/api/reports",{
        method:"get",
        headers: {
    ...
    const results:ActiveReports = data["reports"].map((rep:any)=>{
        return{
            Item:{
